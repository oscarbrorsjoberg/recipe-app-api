...
class PrivateRecipeApiTestCase(TestCase):
    ...
    def test_partial_update_recipe(self):
        ...
        recipe.refresh_from_db()  # update from database
        tags = recipe.tags.all()
...
class RecipeImageUploadTestCase(TestCase):
    ...
    def test_upload_image_bad_request(self):
        """Test for uploadin an invalid image"""
        url = image_upload_url(self.recipe.id)
    ...
    def test_filter_recipes_by_tags(self):
        ''' Test returning recipes with spec tags '''
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Mapo doufu')
        recipe2 = sample_recipe(user=self.user, title='Pancake')
        ingre1 = sample_ingredient(user=self.user, name='Sichuan pepar')
        ingre2 = sample_ingredient(user=self.user, name='Egg')
        recipe1.ingredients.add(ingre1)
        recipe2.ingredients.add(ingre2)

        recipe3 = sample_recipe(user=self.user, title='Gravel')

        res = self.client.get(
            RECIPES_URL,
            {'ingredients': f'{ingre1.id},{ingre2.id}'}
        )
